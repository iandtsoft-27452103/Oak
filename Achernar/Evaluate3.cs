...
namespace Achernar
    // Evaluate2とほぼ同等だが、特徴ベクトルの型をdoubleとした。
{
    internal class Evaluate3
    {
        const short Black = 0;
        const short White = 1;
        const short Empty = 2;
        ...
        public double[,] fv_pb = new double[NSquare, NSquare];
        public double[,] fv_pw = new double[NSquare, NSquare];
        public double[,] fv_bb = new double[NSquare, NSquare];
        public double[,] fv_bw = new double[NSquare, NSquare];// bwとwbは相殺してしまうかもしれないが、取りあえず入れておく。
        public double[,] fv_wb = new double[NSquare, NSquare];
        public double[,] fv_ww = new double[NSquare, NSquare];
        ...
        public double EvaluateAll(Board bt)
        {
            int i, j;
            double score = 0;
            short temp_color = 0;
        ...
        public double EvaluateDiff(Board bt, short color, short move, short prev_value, ref List<short> li_b, ref List<short> li_w)
        {
            int i, j, k, index, index2;
            double score = prev_value * 32;
