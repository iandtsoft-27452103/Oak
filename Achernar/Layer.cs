...
namespace Achernar
{
    internal class Layer
    {
        ...
        public class ResNet : Module<Tensor, Tensor>
        {
            ...
            private readonly long[] planes = new long[] { 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256 };
            private readonly long[] strides = new long[] { 1, 2, 1, 2, 1, 2, 1, 1, 1, 1, 1, 2, 1 };

            private readonly int stride2 = 3;

            private readonly Module<Tensor, Tensor> layers;
            private int in_planes = 256;//元は64
            //private int in_planes2 = 256;

            public static ResNet ResNetCustom(int numClasses, Device device = null)
            {
                return new ResNet(
                    "ResNetCustom",
                    (name, in_planes, planes, stride) => new BasicBlock(name, in_planes, planes, stride),
                    BasicBlock.expansion, new int[] { 2, 2, 2, 2, 2, 2, 2, 2, 2, 2 },
                    numClasses,
                    device);
            }
            ...
            public ResNet(string name, Func<string, int, int, int, Module<Tensor, Tensor>> block, int expansion, IList<int> num_blocks, int numClasses, Device device = null) : base(name)
            {
                ...
                modules.Add(($"conv2d-first", Conv2d(256, 256, kernel_size: 3, stride: 1, padding: 1, bias: false)));
                //modules.Add(($"bnrm2d-first", BatchNorm2d(64)));
                modules.Add(($"bnrm2d-first", BatchNorm2d(256, eps: 2e-05)));
                modules.Add(($"relu-first", ReLU(inplace: true)));
                MakeLayer(modules, block, expansion, 256, num_blocks[0], 1);
                MakeLayer(modules, block, expansion, 256, num_blocks[1], 1);
                MakeLayer(modules, block, expansion, 256, num_blocks[2], 1);
                MakeLayer(modules, block, expansion, 256, num_blocks[3], 1);
                MakeLayer(modules, block, expansion, 256, num_blocks[4], 1);
                MakeLayer(modules, block, expansion, 256, num_blocks[5], 1);
                MakeLayer(modules, block, expansion, 256, num_blocks[6], 1);
                MakeLayer(modules, block, expansion, 256, num_blocks[7], 1);
                MakeLayer(modules, block, expansion, 256, num_blocks[8], 1);
                MakeLayer(modules, block, expansion, 256, num_blocks[9], 1);
